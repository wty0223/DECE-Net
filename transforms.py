...
class Scale(object):
    ...
    def __call__(self, img,contour, label):
        '''
        :param img: RGB image
        ...
        contour = cv2.resize(contour, (self.w, self.h))
        label = cv2.resize(label, (self.w, self.h), interpolation=cv2.INTER_NEAREST)


        return [img,contour, label]
...
class RandomFlip(object):
    ...
    """
    def __call__(self, image,contour, label):
        if random.random() < 0.5:
...
class RandomScale(object):
    ...
    def __call__(self, img,contour, label):
        ...
        # nearest neighbour interpolation for label image
        contour = cv2.resize(contour, (0, 0), fx=scale, fy=scale)
...
class Normalize(object):
    ...
    def __call__(self, image,contour, label):
        ...
        contour=(contour/255).astype(np.float32)
        label = label / 255
...
class ToTensor(object):
    ...
    def __call__(self, image,contour, label):
        if self.scale != 1:
            ...
            contour = cv2.resize(contour, (int(w), int(h)))
        ...
        contour_tensor =torch.from_numpy(contour)
        label_tensor =  torch.LongTensor(np.array(label, dtype=np.int))

        return [image_tensor,contour_tensor, label_tensor]
