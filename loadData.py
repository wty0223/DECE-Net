...
class LoadData_liver(object):
    ...
    def readFile(self, fileName, trainStg=False):
        ...
        with open(osp.join(self.data_dir, fileName), 'r') as text_file:
            for line in text_file:
                ...
                # <RGB Image>, <Label Image>
                line_arr = line.split()
                img_file = osp.join(self.data_dir+'liver/train/', line_arr[0].strip()+'.png')
                label_file = osp.join(self.data_dir+'liver/train/', line_arr[0].strip()+'_mask.png')
                label_img = cv2.imread(label_file, 0) / 255
    ...
    def readFile_val(self, fileName, trainStg=False):
        if trainStg == True:
            global_hist = np.zeros(self.classes, dtype=np.float32)
        ...
        with open(osp.join(self.data_dir, fileName), 'r') as text_file:
            for line in text_file:
                ...
                # <RGB Image>, <Label Image>
                line_arr = line.split()
                img_file = osp.join(self.data_dir+'liver/val/', line_arr[0].strip()+'.png')
                label_file = osp.join(self.data_dir+'liver/val/', line_arr[0].strip()+'_mask.png')
                label_img = cv2.imread(label_file, 0) / 255
    ...
    def processData(self, crossVal, dataset):
        print('Processing training data')
        return_train = self.readFile(dataset + '/train.txt', True)

        print('Processing validation data')
        return_val = self.readFile_val(dataset + '/val.txt')
