...
def train(args, train_loader, model, criterion, optimizer, epoch, max_batches, cur_iter=0):
    ...
    for iter, (input,contour, target) in enumerate(train_loader):
        start_time = time.time()
...
def main_tr(args, crossVal):
    ...
    # load the model
    model = net.Net(args.classes, aux=True)
    ...
    # criteria = TverskyLoss(weight) #weight
    if args.gpu and torch.cuda.device_count() > 1 :
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savedir', default='./new_results_crossVal', help='Directory to save the results')#/media/qiuyu/qiuyu/COVID
