...
def compute_iou(SEG, GT):
    TP = np.sum(np.logical_and(SEG, GT))
    a=np.sum(np.logical_or(SEG,GT))
...
def evaluation_sample(SEG_np, GT_np):
    ...
    quality["iou"]=compute_iou(SEG_np,GT_np)
    ...
    for i in range(crossVal):
        ...
        for idx in range(len(gt_files)):
            ...
            if np.max(pred) != 0:
                ...
                iou[idx]=evaluation_sample(pred,gt).get("iou")
        ...
        iou=np.max(np.mean(iou,axis=0))
        #F_beta = (1 + 0.3) * precision * recall / (0.3 * precision + recall + EPSILON)
        print(i,"{:.4f}".format(sensitive),"{:.4f}".format(specificity),"{:.4f}".format(dice),"{:.4f}".format(hausdorff_distance),"{:.4f}".format(iou))
