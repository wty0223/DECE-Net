...
class Net(nn.Module):
    def __init__(self, classes=2, P1=2, P2=3, P3=8, P4=6, aux=True):
        ...
        self.long1 = DownsamplerBlock(3, 64, stride=2)
        self.down1 = ConvBlock(3, 64, stride=2)
        self.level1 = nn.ModuleList()
        self.level1_long = nn.ModuleList()
        for i in range(0, P1):
            self.level1.append(ConvBlock(64, 64))
        for i in range(0, self.D1):
            self.level1_long.append(DownsamplerBlock(64, 64, stride=1))

        self.cat1 = nn.Sequential(
                    nn.Conv2d(128, 128, 1, stride=1, padding=0, groups=1, bias=False),
                    nn.BatchNorm2d(128))

        self.long2 = DownsamplerBlock(64, 128, stride=2)
        self.down2 = DilatedParallelConvBlock(64, 128, stride=2)
        self.level2 = nn.ModuleList()
        self.level2_long = nn.ModuleList()
        for i in range(0, P2):
            self.level2.append(DilatedParallelConvBlock(128, 128))
        for i in range(0, self.D2):
            self.level2_long.append(DownsamplerBlock(128, 128, stride=1))

        self.cat2 = nn.Sequential(
                    nn.Conv2d(256, 256, 1, stride=1, padding=0, groups=1, bias=False),
                    nn.BatchNorm2d(256))

        self.long3 = DownsamplerBlock(128, 256, stride=2)
        self.down3 = DilatedParallelConvBlock(128, 256, stride=2)
        self.level3 = nn.ModuleList()
        self.level3_long = nn.ModuleList()
        for i in range(0, P3):
            self.level3.append(DilatedParallelConvBlock(256, 256))
        for i in range(0, self.D3):
            self.level3_long.append(DownsamplerBlock(256, 256, stride=1))

        self.cat3 = nn.Sequential(
                    nn.Conv2d(512, 512, 1, stride=1, padding=0, groups=1, bias=False),
                    nn.BatchNorm2d(512))

        self.long4 = DownsamplerBlock(256, 512, stride=2)
        self.down4 = DilatedParallelConvBlock(256, 512, stride=2)
        self.level4 = nn.ModuleList()
        self.level4_long = nn.ModuleList()
        for i in range(0, P4):
            self.level4.append(DilatedParallelConvBlock(512, 512))
        for i in range(0, self.D4):
            self.level4_long.append(DownsamplerBlock(512, 512, stride=1))
        ...
        if self.aux:
            self.pred4 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(512, classes, 1, stride=1, padding=0))
            self.pred3 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(256, classes, 1, stride=1, padding=0))
            self.pred2 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(128, classes, 1, stride=1, padding=0))
        self.pred1 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(64, classes, 1, stride=1, padding=0))
